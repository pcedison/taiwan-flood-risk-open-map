...
US-004：使用者可以看到即時風險、歷史購屋參考風險與資料可信度。\
...
US-010：系統可以用預先計算的行政區或網格風險 profile，讓第一次被搜尋的地點也能快速取得可解釋的初步風險參考，並在背景補做精準半徑查詢。
...
- Public beta read path should prefer precomputed township/village and H3/geohash
  risk profiles before falling back to expensive exact-radius evidence queries.
...
- TGOS optional geocoding/provider adapter, future only
- 中央氣象署 API
- 水利署 API/KML/CSV/SHP
...
- Use MapLibre GL JS.
- Use PMTiles/Protomaps-compatible OpenStreetMap-derived basemap data.
- Serve prebuilt Taiwan/region PMTiles or a licensed hosted MapLibre style JSON
  from project-controlled object storage/CDN, such as Cloudflare R2 or an
  S3-compatible service.
- Keep risk overlays, query heat, flood potential, and project MVT layers
  separate from the basemap provider.

Production rule:

- Do not directly depend on `tile.openstreetmap.org` or other public OSM
  community tile endpoints in production.
- Public OSM tiles are only for local development, prototypes, or explicit
  temporary incident fallback while respecting upstream policy.
- Visible OSM attribution, ODbL notes, source metadata, range request checks,
  cache behavior, and rollback must be documented before public launch.

Deferred higher-ops path:

- OpenMapTiles, Tegola, PostGIS/ST_AsMVT, Martin, or another tile server can be
  added later when traffic, styling, or dynamic layer needs justify generation,
  refresh, cache invalidation, and monitoring cost.
...
- TGOS adapter as a Taiwan-local provider after credential, terms, cost,
  reliability, and attribution review. TGOS is not an MVP or public launch
  blocker.
...
### 7.3 Precomputed area and grid risk profiles

Cold public beta lookups must not depend solely on doing every expensive
radius query synchronously at user request time. The system should maintain a
derived profile layer that can be read quickly when a location has not been
queried before.
...
- `town`
- `village`
- `h3` or `geohash` grid cells, initially 500m-1km equivalent resolution

Neighbor-level (`lin`/鄰) profiles are deferred until reliable open boundary
data, privacy density review, and public explanation wording are accepted.
Until then, village plus grid cells are the finest precomputed public profile
scopes.

Profile records are derived summaries, not source evidence. A profile must keep:

- stable area/grid key
- geometry or centroid
- profile radius or polygon/buffer rule, with 2km as the first public beta
  aggregation radius for village/township previews
- score version
- realtime level when applicable
- historical level
- confidence level
- evidence counts by source type and event type
- top evidence IDs for explanation
- latest observed, occurred, and ingested timestamps
- coverage gaps and missing-source warnings
- `computed_at`, `expires_at`, and stale/healthy status

Read path:

1. Geocode or map click returns a point.
2. API resolves the point to village/township and grid cell.
3. API reads the freshest matching profile and returns it as the fast initial
   risk context when exact radius evidence is cold or slow.
4. API or worker enqueues an exact-radius refresh for the specific point/radius.
5. Profile refresh jobs update affected area/grid profiles after new evidence is
   promoted.

Profile scoring must be evidence-backed. Query heat may influence refresh
priority and review priority, but must not directly raise the flood risk level.

### 7.4 Vector-assisted evidence intelligence

The primary storage and retrieval model remains PostGIS plus relational evidence
tables. Vector embeddings may be added as an auxiliary layer only for:

- public-news deduplication and near-duplicate clustering;
- semantic relevance ranking for flood-related article titles/summaries;
- fuzzy location-text matching when road/village terms are ambiguous;
- reviewer triage and candidate grouping.

Vector similarity alone is never sufficient to create or raise flood-risk
evidence. Every vector-derived candidate must retain source URL, title or
metadata, evidence ID, model/version, confidence, and a PostGIS geometry or
explicit location uncertainty. Embeddings do not replace source retention,
license, takedown, or explainability obligations.
...
不可直接提高 `realtime_score` 或 `historical_score`。Query heat may only
prioritize profile refresh, source backfill, operator review, caching, and
public attention display. 若未來要讓 query heat 影響模型，必須新增 ADR，並設計防操縱機制。
...
- `evidence_embeddings` optional, auxiliary only
- `map_layers`
- `ingestion_jobs`
- `adapter_runs`
- `user_reports`
...
- `admin_area_profiles.area_key BTREE`
- `admin_area_profiles.geom GIST`
- `risk_grid_profiles.grid_key BTREE`
- `risk_grid_profiles.geom GIST`
- `profile_evidence_links.evidence_id BTREE`
- `profile_refresh_jobs.status, priority BTREE`
- `evidence_embeddings.evidence_id BTREE`
...
- Area/grid risk profiles：保留 derived summary and evidence references；when
  stale they must be marked stale instead of silently treated as current.
- Embeddings：must be deletable with their source evidence and must not contain
  raw article text beyond the reviewed metadata/summary policy.
...
- Do not run a full production map tile server yet.
- Use the open basemap PMTiles path: prebuilt Taiwan/region PMTiles or a
  licensed hosted style JSON served from object storage/CDN.
- Runtime smoke and private beta must not require TGOS.
...
- Add object storage for raw snapshots, backups, and versioned PMTiles/style
  assets.
- Add monitoring.
- Keep CDN/object-storage basemap delivery as the default unless map traffic or
  styling needs justify a tile server.

Stage C：Public production

- Separate app server, database server, object storage, CDN, and backup storage.
- Add a dedicated tile server only if the accepted operating plan covers tile
  generation, refresh cadence, cache invalidation, monitoring, and rollback.
...
- Open basemap PMTiles/object-storage/CDN runbook
- Tile pipeline only when accepted as a higher-operations follow-up
- Monitoring
- Backup/restore
- Load test
- Security review
...
- Open basemap PMTiles/style configuration
- OSM-derived data attribution and ODbL metadata
- Layer metadata
- TileJSON endpoints
- Map styles
...
- Attribution visible.
- PMTiles/object-storage/CDN range request, cache, and rollback checks are
  documented.
- Runtime smoke does not require TGOS.
- Tile server documented only if the team accepts the higher-ops tile server
  path.
...
- Browser can load a project-controlled or explicitly licensed MapLibre style
  without depending on TGOS.
- Basemap uses PMTiles/Protomaps-compatible OSM-derived data through
  object-storage/CDN or a licensed hosted style path; production must not
  directly hit public OSM tile endpoints.
...
- Documentation explains limitations, non-guarantee, OSM/ODbL attribution,
  range request checks, cache behavior, and rollback.
...
OQ-002：TGOS 是否為 MVP 或上線必要 provider
Decision：TGOS 保留為 future optional Taiwan-local provider。自架或開源
geocoder、Map click query、以及 open basemap PMTiles 主線優先；TGOS 不作為
MVP 或公益服務上線阻塞。
...
OQ-011：如何降低 public beta 第一次查詢延遲
Decision：新增行政區與 H3/geohash 預先風險 profile 作為冷查 fast path。
Profile 以 accepted evidence、官方淹水潛勢、歷史新聞與資料新鮮度計算；
查詢熱度只可提升 profile refresh/review priority，不可直接提升風險。
向量資料庫/embedding 只作新聞語意去重、相似度排序與地名輔助，不取代
PostGIS 空間查詢或 evidence-backed scoring。
...
- ADR-0002：MapLibre + PMTiles open basemap
