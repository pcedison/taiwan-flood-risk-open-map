...
**2026-07-06 現況的結論**：完全不受 secrets 空清單影響，會正常運作。
...
| `local-source-dispatch-watchdog.yml` | 0 | 不受影響，正常運作。 |

**整體結論**：四個 workflow 中有兩個（schedule watchdog、local source dispatch watchdog）完全不需要任何 repo secret；一個（secret readiness watchdog）本身就是用來監控另一個 workflow 的 secret 是否齊全，目前會誠實回報「5 個都沒設」但不會讓自己失敗；只有 `hosted-monitoring.yml` 真正需要這 5 個 secrets 才能收集完整的私有 hosted 證據，2026-07-06 稽核時這 5 個全部缺席，代表**私有 hosted 證據鏈目前是空的**，只有公開端點層級的冒煙測試在跑。
