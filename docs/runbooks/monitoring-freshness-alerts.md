...
- Prometheus rule coverage for freshness, API readiness, and future heartbeats.
...
Worker and scheduler heartbeat alerts are present as non-firing future
placeholders. They must stay non-firing until runtime heartbeat timestamp
...
| `FloodRiskWorkerHeartbeatMissing` | warning | future metric | Non-firing placeholder until worker heartbeat exists. |
| `FloodRiskSchedulerHeartbeatMissing` | warning | future metric | Non-firing placeholder until scheduler heartbeat exists. |
...
When runtime heartbeat metrics are implemented, update
`infra/monitoring/alert-rules.yml` by replacing the placeholder expressions:
