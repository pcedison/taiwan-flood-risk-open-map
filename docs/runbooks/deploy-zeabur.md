...
The checked-in production readiness evidence YAML is a template only. This repo
does not store real Zeabur secrets, private alert routes, or production drill
artifacts. Keep private production evidence in an ops-controlled location and
validate it with:
...
Use this path for the current Zeabur preview. It runs the Next.js web UI and
FastAPI API in one container from the repository root `Dockerfile`. TGOS is now
a future optional Taiwan-local provider, not a blocker for this deployment path;
the launch basemap path is MapLibre with a reviewed open style or PMTiles source.
...
| `NEXT_PUBLIC_BASEMAP_STYLE_URL` | reviewed style URL or blank | Preferred open basemap path. This is a client build-time value, so rebuild after changing it. |
| `NEXT_PUBLIC_BASEMAP_KIND` | `pmtiles`, `raster`, or blank | Use `pmtiles` with a reviewed PMTiles URL; blank keeps local/dev fallback only. |
| `NEXT_PUBLIC_BASEMAP_PMTILES_URL` | reviewed PMTiles URL or blank | Must point to a provider/package with license, attribution, range-request, and update-cadence evidence. |
| `NEXT_PUBLIC_BASEMAP_RASTER_TILES` | reviewed raster tile template or blank | Temporary fallback only; do not use public OSM community tiles for production. |
...
`NEXT_PUBLIC_BASEMAP_*` evidence instead.
...
| `OFFICIAL_NATIONWIDE_HISTORY_CITATIONS_ENABLED` | api | Default `false` since 2026-09-04. Request-time Google News/Bing citation lookup of government pages; leave off unless the precision-weighted scorer and title-based location matching are deployed. Persisted rows from this source are excluded from scoring while off |
...
| `SOURCE_DCARD_ENABLED` | worker, scheduler | Source-level Dcard gate; also requires `SOURCE_FORUM_ENABLED=true`, terms acknowledgement, and candidate approval ack |
| `SOURCE_PTT_CANDIDATE_APPROVAL_ACK` | worker, scheduler | Allows only the no-network synthetic PTT candidate fixture contract; not real ingestion approval |
...
6. Taiwan-wide public smoke passes:
   `python scripts\taiwan_wide_public_beta_smoke.py --base-url https://floodrisk.cc --include-town-samples --request-delay-seconds 2.2 --rate-limit-retries 8 --rate-limit-retry-delay-seconds 10`.
...
- Confirm production readiness checklist was run for production beta:
  `python infra/scripts/validate_monitoring_assets.py` and
  `python infra/scripts/validate_production_readiness_evidence.py --production-complete <private-evidence.yaml>`.
