...
python scripts/local-source-request-packets.py --format markdown --signal-type pump_or_gate_status
...
`--signal-type` 可用來產生單一訊號缺口批次，例如目前最大的
`pump_or_gate_status` 批次；它仍會保留該訊號相關的授權、metadata release、
public API contract 與 signal-gap 請求包。
