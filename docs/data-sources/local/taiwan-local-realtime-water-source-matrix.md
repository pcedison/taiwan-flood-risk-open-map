...
| 臺北市 | 疏散門/水門狀態 | 臺北市資料大平臺「臺北市疏散門即時監測」 | JSON API：`https://wic.heo.taipei/OpenData/API/Evacuate/Get?stationNo=&loginId=watergate&dataKey=44D76DA6` | `recTime`，官方欄位說明 | 官方頁公開 loginId/dataKey；無額外人工申請 | `needs_review` | 官方 OpenAPI contract 已知；2026-06-28 從目前環境 smoke 30 秒 timeout，暫不進 production adapter，待 live availability/freshness 複核。 |
...
| 臺南市 | 淹水感測器 | data.gov.tw dataset `128983` | JSON API：`https://soa.tainan.gov.tw/Api/Service/Get/21b31a27-3e61-48b8-8259-83c2001bec8c`；metadata：`https://soa.tainan.gov.tw/Api/Service/Get/cdc1ead4-d56a-4092-8e1c-e1f2fa9ee864` | `InfoTime` | 不需 token | `ready_implemented` | 已有 `local.tainan.flood_sensor`，仍需與後續地方框架對齊。 |

## 22 縣市地方來源覆蓋矩陣

| 縣市 | 目前地方來源狀態 | 最佳候選 | 缺口 / 下一步 |
| --- | --- | --- | --- |
| 臺北市 | `ready_implemented` + `needs_review` | 雨水下水道水位、河川水位、抽水站即時資訊；疏散門即時監測 OpenAPI | 已新增三個 local adapter，含 metadata join、逐站 stale filter、抽水站外水位 metric。疏散門 OpenAPI contract 已知但目前 live smoke timeout，需複核後再實作。 |
...
| 苗栗縣 | `ready_implemented` + `candidate` + `central_aggregated_ready` | FHY Broker 苗栗縣政府淹水感測器；Civil IoT `water_12` 苗栗淹水感測器；Civil IoT/STA_RainSewer 雨水下水道水位；雨水下水道水位監測計畫 | 已新增 `local.miaoli.flood_sensor`；FHY CityCode 10005、Supplier=苗栗縣政府 42 站，2026-06-28 live adapter fetched 42、normalized 40、stale reject 2。苗栗雨水下水道水位監測仍未公開 read API contract。 |
...
| 臺南市 | `ready_implemented` | 淹水感測器 JSON | 已接入；下一步加入與地方框架一致的 coverage metadata。 |
| 高雄市 | `ready_implemented` + `central_aggregated_ready_implemented` | 高雄 SFC sewer/rt、SFC 淹水感測；Civil IoT 淹水感測器、STA_RainSewer、Civil IoT 抽水站；高雄閘門及抽水站靜態清冊 | 已新增 `local.kaohsiung.sewer_water_level` 與 `local.kaohsiung.flood_sensor`；雨量 API 缺站名/座標暫不納入，抽水站/水門靜態清冊仍作 metadata 背景。 |
| 屏東縣 | `ready_implemented` + `central_aggregated_ready_implemented` + `candidate` + `needs_review` | FHY Broker 屏東縣政府淹水感測器；Civil IoT 淹水感測器、STA_RainSewer、Civil IoT 抽水站/閘門；屏東防災平台 `/RainStation`、`/River`、`/Flood`、`/Crawler` | 已新增 `local.pingtung.flood_sensor`；FHY CityCode 10013、Supplier=屏東縣政府 20 站，2026-06-28 live adapter fetched/normalized 20。PTEOC HTML 仍缺觀測時間與座標，不納入 production。 |
...
| 臺東縣 | `ready_implemented` + `central_aggregated_ready` + `candidate` | FHY Broker 臺東縣政府淹水感測器；Civil IoT 淹水深度、Civil IoT/STA_RainSewer 雨水下水道水位、WRA 即時水位；洪水/淹水預警系統 | 已新增 `local.taitung.flood_sensor`；FHY CityCode 10014、Supplier=臺東縣政府 2 站，2026-06-28 live adapter fetched/normalized 2。臺東洪水與淹水預警系統仍未公開 read API endpoint。 |
...
| 金門縣 | `central_aggregated_ready` + `needs_application` | Civil IoT 淹水感測器、STA_RainSewer；KWIS 第三方設備上傳 API | 2026-06-28 嚴格查核：KWIS 是 SOAP/ASMX 上傳介接，需線上申請帳密/key/token，非公開查詢 API；不作 production ingestion。Civil IoT live smoke：淹水感測 7 站、RainSewer 29 站，可作中央主幹補強但不等於地方政府 read API。 |
| 連江縣 | `not_found` + `metadata_only` + `needs_hydrologic_backbone` | CWA 雨量、NCDR CAP；大潮、豪雨易淹水地區 ODS | 2026-06-28 嚴格查核：只找到靜態 ODS/防災資料，無地方 live API；RainSewer live smoke count=0，中央主幹目前仍缺水位/淹水深度/下水道/抽水站/閘門等水文觀測訊號。 |
...
| 苗栗縣 | `ready_implemented` + `candidate` | FHY Broker 苗栗縣政府淹水感測器；苗栗縣水利處雨水即時水情監測成果 | SOAP/ASMX JSON POST；雨水下水道監測目前只有新聞/成果說明 | FHY 不需 | 2026-06-28 smoke：CityCode 10005，Supplier=苗栗縣政府 42 站，normalized 40、stale reject 2 | 已新增 `local.miaoli.flood_sensor`；雨水下水道監測系統等待公開 API。 |
...
| 高雄市 | `ready_implemented` | 高雄市智慧水利監測密網平台、水情 e 點靈 | JSON | 不需 | 2026-06-28 smoke：sewer/rt 296 筆、wrs_flooding_sensor 171 筆，均含時間與座標 | 已新增 `local.kaohsiung.sewer_water_level`、`local.kaohsiung.flood_sensor`。 |
| 屏東縣 | `ready_implemented` + `candidate` + `needs_review` | FHY Broker 屏東縣政府淹水感測器；屏東防災資訊整合平台、縣府智慧防災新聞 | SOAP/ASMX JSON POST；HTML/平台服務；`/RainStation` 可讀雨量表但缺觀測時間與座標 | FHY 不需；PTEOC 未公開 API | 2026-06-28 smoke：FHY CityCode 10013，Supplier=屏東縣政府 20 站，normalized 20；PTEOC HTML 不足以 production | 已新增 `local.pingtung.flood_sensor`；PTEOC 只列觀察線索。 |
...
| 臺東縣 | `ready_implemented` + `candidate` | FHY Broker 臺東縣政府淹水感測器；臺東縣府防汛新聞、審計部臺東水情預警系統說明 | SOAP/ASMX JSON POST；預警系統未公開 endpoint | FHY 不需 | 2026-06-28 smoke：FHY CityCode 10014，Supplier=臺東縣政府 2 站，normalized 2 | 已新增 `local.taitung.flood_sensor`；臺東預警系統等待公開 API。 |
...
| 金門縣 | `needs_application` | 金門水情系統、API 介接申請及使用說明 PDF | SOAP/ASMX 上傳服務 | 需縣府審核帳密/key；token 10 分鐘 | `Measurement_datetime`、`Operation_datetime` | 非公開 read API；需人工申請且用途是設備上傳。 |
| 連江縣 | `metadata_only` | 連江縣開放資料查詢、防救災資訊網 | ODS 靜態易淹水地區資料 | 不需 | 無即時欄位 | 不新增 local adapter。 |
...
18. `local.yilan.flood_sensor`（已實作）
19. `local.yilan.water_level`（已實作）
20. `local.keelung.water_level`（已實作）
21. `local.keelung.flood_sensor`（已實作）
22. `local.keelung.rainfall`（已實作；stale 站保留 raw、normalized reject）
23. `local.yunlin.water_level`（已實作；淹水感測 depth 欄位仍待查）
24. `local.new_taipei.water_level`（已實作）
25. `local.new_taipei.flood_sensor`（已實作）
26. `local.new_taipei.rainfall`（已實作；stale 站保留 raw、normalized reject）
27. `local.new_taipei.drainage_water_level`（已實作）
28. `local.penghu.water_level`（已實作；ArcGIS epoch 需扣 8 小時後判讀）
29. `local.hsinchu_county.flood_sensor`（已實作；FHY local-government supplier only）
30. `local.miaoli.flood_sensor`（已實作；FHY local-government supplier only）
31. `local.changhua.flood_sensor`（已實作；FHY local-government supplier only）
32. `local.pingtung.flood_sensor`（已實作；FHY local-government supplier only）
33. `local.hualien.flood_sensor`（已實作；FHY local-government supplier only）
34. `local.taitung.flood_sensor`（已實作；FHY local-government supplier only）

## 下一批候選

1. 屏東 HTML 來源：`/RainStation`、`/Flood`、`/WaterStationVideo`、`/Crawler` 均可讀，但目前仍需穩定 HTML parser、觀測時間與座標 join；未解決前維持 `needs_review`。
...
3. 花蓮、金門：花蓮 FHY local adapter 已可運作，但 Senslink 登入型儀表板仍需帳密、
   key、token 或官方授權；金門 KWIS 仍需要人工申請或官方公開 read API。
4. 苗栗、臺東：FHY local adapter 已可運作；官方說明中的其他系統仍未找到公開 API contract。
...
- Admin API `GET /admin/v1/local-source-coverage` 會輸出 22 縣市地方來源
  ...
  抽水站、閘門或埤塘水位等水文觀測。連江目前 RainSewer count=0，coverage
  level 會顯示 `needs_hydrologic_backbone`，避免被 CWA/NCDR 基礎訊號誤判為
  水文觀測完整；頂層 `central_backbone_required_families`、
