...
每個地方 adapter 的授權條款現況（已查證/推定待查證/與官方聲明衝突等）逐一盤點
見 `docs/data-sources/local/LICENSE_TERMS.md`；在完成該文件第 2 節的查證步驟前，
不應把任何地方來源當作已授權的開放資料視為理所當然。
...
- 2026-07-01 `scripts/local-source-candidate-smoke.py --allow-insecure-tls`
  已將 21 個缺口候選寫入
  `docs/reviews/local-source-candidate-smoke-2026-07-01.json`。結果為：
  `promotion_ready` 1 筆（高雄雨量，已有 adapter）、
  `status_only_ready` 1 筆（雲林淹水感測狀態，不可當淹水深度）、
  `needs_measurement_value` 1 筆（臺北疏散門/水門狀態，缺量測值）、
  `blocked_fetch_error` 1 筆（嘉義縣頁面 TLS/DH 阻擋，需人工或替代探測），
  其餘 17 筆仍缺真正觀測時間、站點/座標 join 或量測值。這輪沒有新增可直接
  start-adapter-TDD 的地方即時感測來源。
