...
| 連江縣 | `not_found` + `metadata_only` + `needs_hydrologic_backbone` + `non_qualifying` | CWA 雨量、NCDR CAP；大潮、豪雨易淹水地區 ODS；連江自來水廠水庫水位月報；連江縣資訊公開查詢系統即時監測值 | 2026-06-28 嚴格查核：只找到靜態 ODS/防災資料，無地方 live API；RainSewer live smoke count=0。2026-06-30 追加查核：自來水廠水庫水位為月報 PDF，`erbwater` 即時監測值為放流水環保 CEMS，兩者都不可補作淹水、水位、雨水下水道、抽水站或水門即時觀測。中央主幹目前仍缺 hydrologic_observation。 |
...
5. 連江：連江自來水廠水庫水位月報與 `erbwater` 即時監測頁已查核；前者是月報
   PDF，後者是放流水環保 CEMS，均列為 `non_qualifying`，不可轉作
   production adapter 或補足 hydrologic_observation。
...
- Admin API `GET /admin/v1/local-source-coverage` 會輸出 22 縣市地方來源
  ...
  抽水站、閘門或埤塘水位等水文觀測。連江目前 RainSewer count=0；水庫水位
  月報與放流水 CEMS 只能作排除紀錄，不能滿足此門檻。coverage level 會顯示
  `needs_hydrologic_backbone`，避免被 CWA/NCDR 基礎訊號誤判為水文觀測完整；
