...
中央政府來源（CWA/WRA/Civil IoT/地理編碼）的授權在
`docs/data-sources/official/official-source-catalog.yaml` 有完整記載，每筆都附
`data_gov_dataset_id`、`data_gov_url`、`license` 三個可追溯欄位。地方政府來源
...
CAP），1 列是 FHY Broker 的彙總描述（實際展開為 6 個縣市、6 個獨立 adapter
key）。本文件以程式碼實際存在的 35 個 `local.*` adapter key 為準逐一列出，
比矩陣原始列數（35，含 5 列中央訊號）在「地方政府項目」的顆粒度上更細（FHY
展開成 6 列），因此本文件的地方來源列數與矩陣列數不會逐行對應，差異原因即
在此；金門 KWIS 雖不在矩陣 51–87 行的「第一批」表格內（該表格只收錄已升級為
...
| 已查證但有但書 | 3 |（併入上列臺北市 3 筆，見第 5.1 節說明） |
| 平臺聲明存在，端點未逐筆核對 | 1 | 臺中市 1 |
| 推定，待查證 | 12 | 基隆市 3、新竹市 1（sewer）、南投縣＊、雲林縣 1、嘉義縣 1、宜蘭縣 2、高雄市＊、其他見下表 |
| 與官方聲明衝突 | 8 | 南投縣 1、FHY 廣播來源 6（新竹縣/苗栗/彰化/屏東/花蓮/臺東 flood_sensor）、高雄市 3（見說明，重複計列於下） |
| 官方誠實標示未定位（程式碼已誠實） | 5 | 新北市 4、澎湖縣 1 |
| 需機關授權（已知） | 1 | 金門縣 KWIS（候選，未啟用） |

（＊南投縣、高雄市同時出現在「推定」與「與官方聲明衝突」是因為：程式碼標註
為政府資料開放授權條款推定值，但官方首頁查到的是版權保留聲明，因此最終分類
以「與官方聲明衝突」為準，上面加註僅為交叉說明，不重複計入總數。實際不重複
總數：已查證 9＋平臺聲明未核對 1＋推定待查證 8＋與官方聲明衝突 8＋誠實標示
未定位 5＋需機關授權 1＋FHY 展開後獨立計列已併入上列 6 筆＝35。）
...
### 5.8 FHY Broker 共用來源（6 個 adapter：新竹縣、苗栗、彰化、屏東、花蓮、臺東）

`FHY_DATA_URL = "https://www.dprcflood.org.tw/SGDS/"`（財團法人成大水利研究
中心受經濟部水利署委託建置的「防災協作平台」廣播端點）。這 6 個 adapter 共用
...
依據前，不應將這 6 個 adapter 視為已授權的開放資料來源。**
...
2. **FHY Broker 共用來源（6 個 adapter：新竹縣/苗栗/彰化/屏東/花蓮/臺東）**
   ——官方聲明版權保留，涉及面最廣（一次影響 6 個縣市的 flood_sensor 訊號）。
