...
def test_settings_can_disable_tainan_official_history_news(monkeypatch):
    get_settings.cache_clear()
    monkeypatch.setenv("APP_ENV", "production-beta")
    monkeypatch.setenv("OFFICIAL_TAINAN_HISTORY_NEWS_ENABLED", "false")
...
def test_settings_enables_short_risk_response_cache_for_hosted_runtime(monkeypatch):
    ...
    assert settings.geocode_cache_backend == "redis"
    assert settings.geocode_cache_ttl_seconds == 86400
...
def test_settings_local_defaults_allow_realtime_diagnostic_fallback(monkeypatch):
    ...
    assert settings.geocode_cache_backend == "memory"
