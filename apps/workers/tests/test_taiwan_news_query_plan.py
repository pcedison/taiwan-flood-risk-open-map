...
def test_default_taiwan_flood_news_terms_are_gdelt_safe() -> None:
    joined = " ".join(DEFAULT_TAIWAN_FLOOD_NEWS_QUERIES)

    for too_short in ("淹水", "積水", "豪雨", "暴雨", "水災"):
        assert too_short not in TAIWAN_FLOOD_NEWS_TERMS
    assert '"積淹水"' not in joined
    assert all(len(term.replace("臺", "台").replace(" ", "")) >= 3 for term in TAIWAN_FLOOD_NEWS_TERMS)
...
def test_build_taiwan_flood_news_queries_skips_short_generated_terms() -> None:
    queries = build_taiwan_flood_news_queries(
        ("東路", "中街", "本和里"),
        terms_per_query=2,
    )
    ...
    assert "本和里" in queries[0]
    assert "東路" not in queries[0]
    assert "中街" not in queries[0]
