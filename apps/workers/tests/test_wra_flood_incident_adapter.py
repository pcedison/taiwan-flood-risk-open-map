...
def test_adapter_uses_secret_header_and_normalizes_historical_flood_reports() -> None:
    ...
    assert result.rejected == ("22222222-2222-2222-2222-222222222222",)
    assert result.source_rejections[0].reason_code == "missing_reviewed_point_geometry"
...
def test_runtime_requires_all_three_gates_and_key() -> None:
    ...
    assert len(adapters["official.wra.flood_incident"].run().normalized) == 1
...
def test_wra_incidents_pass_reviewed_historical_staging_contract() -> None:
    ...
    assert batch.rejected_raw_source_ids == (
