...
INPUT_SCHEMA_VERSION = "hosted-worker-policy-evidence-input/v1"
EVIDENCE_SCHEMA_VERSION = "hosted-worker-policy-evidence/v1"
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description=(
            "Validate hosted worker policy/ops evidence and optionally emit a "
            "mergeable local-source-completion-evidence/v1 overlay for the "
    ...
    parser.add_argument(
        ...
        help=(
            "Optional completion evidence overlay path. Requires the three "
            "hosted worker policy requirements to be verified."
        ),
    )
    ...
    if failures:
        print("HOSTED_WORKER_POLICY_EVIDENCE failed")
    ...
    if args.completion_evidence_output:
        _write_json(
            ...
            build_completion_evidence_overlay(
                ...
                policy_evidence=evidence["hosted_worker_policy_evidence"],
            ),
        )

    print("HOSTED_WORKER_POLICY_EVIDENCE passed")
    return 0
...
def build_evidence_artifact(
    *,
    ...
    return {
        ...
        "captured_at": captured_at,
        "status": status,
        "hosted_worker_policy_evidence": policy_evidence,
...
def _requirement_evidence(
    *,
    ...
    for requirement in REQUIREMENTS:
        item = policy_evidence[requirement]
        detail = {
            "requirement": requirement,
            "evidence_ref": (
                f"{evidence_ref}#/hosted_worker_policy_evidence/{requirement}"
            ),
        }
